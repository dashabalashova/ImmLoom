...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a multi-sample block graph from pairwise block tables, "
            "save connected components, optionally refine oversized components, "
            "and export graph visualizations."
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Only draw components with at least this many nodes",
    )
    parser.add_argument(
        "--layout-seed",
        type=int,
        default=42,
        help="Seed for spring layout used in component plots",
    )
    parser.add_argument(
        "--max-nodes-per-group",
        type=float,
        ...
        help=(
            ...
            "refine the component with greedy modularity communities"
        ),
    )
...
def parse_pair_table_name(file_path: Path) -> tuple[str, str]:
    """
    Example:
    ...
    if len(parts) < 3:
        raise ValueError(
            ...
            f"Expected pair_<i>-<sample_x>_<j>-<sample_y>.tsv"
        )

    x_part = parts[1]
    y_part = parts[2]

    group_x = x_part.split("-", 1)[1]
    group_y = y_part.split("-", 1)[1]
...
def build_block_graph(pair_dir: Path, use_multi: bool = False) -> tuple[nx.Graph, list[Path]]:
    ...
    for file_path in files:
        group_x, group_y = parse_pair_table_name(file_path)
...
def refine_components(
    ...
    """
    Iteratively refine oversized components using greedy modularity communities
    until all components satisfy the threshold or no further split is possible.
    """
    ...
    for iteration in range(1, max_iter + 1):
        new_components: list[set[tuple[str, int]]] = []
        changed = False
        ...
        for component in components:
            ...
            # community detection works on simple Graph
            ...
            changed = True
            new_components.extend(set(c) for c in communities)

        components = sorted(new_components, key=len, reverse=True)

        sizes = [len(c) for c in components[:10]]
        print(f"Refinement iteration {iteration}: {len(components)} components, largest sizes: {sizes}")
...
def make_global_layout(
    graph: nx.Graph,
) -> tuple[dict[tuple[str, int], tuple[float, float]], dict[str, int]]:

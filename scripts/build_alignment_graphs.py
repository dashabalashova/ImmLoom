...
def plot_pairwise_identity(identity_df: pd.DataFrame, out_path: Path, component_id: str):
...
def plot_coverage(alignment, out_path: Path, component_id: str):
    records = list(alignment)
    n_seq = len(records)
    aln_len = alignment.get_alignment_length()
...
def main():
    ...
    alignments_dir = Path(f"results/{dataset}/clustalo/{locus}")
    graphs_dir = Path(f"results/{dataset}/components/{locus}/graphs")
    tables_dir = Path(f"results/{dataset}/components/{locus}/tables")
